-- ATH-439: Distinct assigned clients (auth users) per exercise, aligned with assign/plan flows:
-- user_packages + plan_module + module_exercise + user_assignment_session + user_assignment_exercise
-- (same merge idea as get_current_assigned_session_exercises / get_current_assigned_sessions).
  ...
  cross join lateral (
    ...
    effective_module as (
      ...
      left join public.user_assignment_session uas_t
        on uas_t.assignment_id = a.assignment_id
      ...
      join public.user_assignment_session uas_a
        on uas_a.assignment_id = a.assignment_id
