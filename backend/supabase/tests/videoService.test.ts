...
// ✅ Matches what you used successfully in curl: X-Uploader-User-Id: 53
const MOCK_UPLOADER_USER_ID = 53;

function buildMockSupabase({
  uploadError = null as any,
  dbInsertError = null as any,
  dbUpdateError = null as any,
  dbSelectError = null as any,
  publicUrl = MOCK_PUBLIC_URL as string | null,
  exerciseData = null as any,
} = {}) {
  const storageFrom = {
  ...
  const videoInsertChain = {
    insert: jest.fn().mockReturnThis(),
    select: jest.fn().mockReturnThis(),
    single: jest
      .fn()
      .mockResolvedValue({
        data: dbInsertError ? null : { video_id: MOCK_VIDEO_ID },
        error: dbInsertError,
      }),
  };

  const exerciseUpdateChain = {
    update: jest.fn().mockReturnValue({
      eq: jest.fn().mockResolvedValue({ error: dbUpdateError }),
    }),
  };

  const exerciseSelectChain = {
    select: jest.fn().mockReturnThis(),
            ...
            video: [
              {
                ...
                original_filename: 'crunches.mp4',
                mime_type: 'video/mp4',
              },
            ],
          }),
  ...
  return {
    ...
    from: jest.fn((table: string) => {
      if (table === 'video') return videoInsertChain;
      if (table === 'exercise') return { ...exerciseUpdateChain, ...exerciseSelectChain };
      return {};
    }),
    _storageFrom: storageFrom,
    _videoInsertChain: videoInsertChain,
  };
}
...
describe('ATH-246 — Upload Exercise Videos to Supabase Storage', () => {
  ...
  describe('AC2: Video file accessible via public URL', () => {
    ...
    it('should throw if public URL cannot be retrieved after upload', async () => {
      ...
      ).rejects.toThrow('Could not retrieve public URL after upload.');
    });
  });
  ...
  describe('AC3: Exercise record includes a valid video_id field', () => {
    ...
    it('should throw if linking video to exercise fails', async () => {
      ...
      await expect(
        linkVideoToExercise(supabase as any, MOCK_EXERCISE_ID, MOCK_VIDEO_ID)
      ).rejects.toThrow('Failed to link video to exercise: FK violation');
    });

    it('full upload-and-link flow should produce a non-null video_id', async () => {
      const supabase = buildMockSupabase();

      const { video_id } = await uploadExerciseVideo(
        supabase as any,
        Buffer.from('fake'),
        'crunches.mp4',
        'video/mp4',
  ...
  describe('AC4: Stored URL correctly points to the uploaded video', () => {
    ...
    it('public URL from upload should match Supabase storage URL format', async () => {
      ...
      expect(result.publicUrl).toMatch(
        /https:\/\/.+\.supabase\.co\/storage\/v1\/object\/public\/exercise-videos\/.+/
      );
    });
  });
  ...
  describe('AC5: Video URL retrieved when querying the exercise', () => {
    ...
    it('should return null videoUrl if exercise has no linked video', async () => {
      const supabase = buildMockSupabase({
        exerciseData: { exercise_id: MOCK_EXERCISE_ID, title: 'pelvic tilt', video_id: null, video: null },
      });
    ...
    it('should throw if the exercise query fails', async () => {
      ...
      await expect(getExerciseWithVideoUrl(supabase as any, MOCK_EXERCISE_ID)).rejects.toThrow(
        'Failed to fetch exercise: Row not found'
      );
    });
  });
});